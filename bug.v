...
(* File reduced by coq-bug-minimizer from original input, then from 3505 lines to 2115 lines *)
(* coqc version 9.1+alpha compiled with OCaml 4.14.2
   coqtop version runner-t7b1znuaq-project-4504-concurrent-3:/builds/coq/coq/_build/default,(HEAD detached at e9b80a3beb6ab0) (e9b80a3beb6ab0efd39fecb8f9146cb57189133f)
   Expected coqc runtime on this file: 77.373 sec *)
...
Require Crypto.Util.ZUtil.Signed.
...
Require Crypto.Util.FSets.FMapFlip.
Require Crypto.Util.FSets.FMapSum.
Require Crypto.Util.FSets.FMapZ.
...
Require Crypto.Util.ListUtil.SetoidListFlatMap.
Require Crypto.Util.FSets.FMapProd.
Require Crypto.Util.FSets.FMapSect.
Require Crypto.Util.Structures.Equalities.List.
Require Crypto.Util.Structures.Orders.List.
Require Crypto.Util.Tactics.SetEvars.
Require Crypto.Util.Tactics.SubstEvars.
Require Crypto.Util.FSets.FMapTrie.Shape.
Require Crypto.Util.FSets.FMapTrie.
Require Crypto.Util.FSets.FMapTrie.ShapeEx.
Require Crypto.Util.FSets.FMapTrieEx.
Require Crypto.Util.MSets.MSetIso.
Require Crypto.Util.MSets.MSetN.
Require Crypto.Util.ZUtil.Lxor.
...
Require Crypto.Util.Tactics.WarnIfGoalsRemain.
...
Require Crypto.Util.ZRange.SplitBounds.
...
Require Crypto.Util.ZRange.LandLorBounds.
Require Crypto.Util.ZUtil.Ltz.
...
Require Crypto.Util.ZUtil.Tactics.SolveTestbit.
Require Crypto.Util.ZUtil.TruncatingShiftl.
Require Crypto.Util.ZUtil.Zselect.
Require Crypto.Util.ZUtil.Combine.
Require Crypto.Util.Logic.ProdForall.
Require Crypto.CastLemmas.
Require Crypto.AbstractInterpretation.ZRangeCommonProofs.
...
Require Crypto.Util.MSets.MSetSum.
Require Crypto.Util.Strings.NamingConventions.
Require Crypto.Util.ZRange.Show.
Require Crypto.Util.MSets.Show.
Require Crypto.Util.MSets.MSetPositive.Show.
Require Crypto.Util.Bool.Equality.
Require Crypto.Stringification.Language.
Require Crypto.Util.ListUtil.CombineExtend.
Require Crypto.Util.ListUtil.RemoveN.
Require Crypto.Util.ErrorT.List.
Require Crypto.Assembly.Equivalence.
Require Crypto.Util.ListUtil.Split.
Require Crypto.Util.Tactics.RevertUntil.
Require Crypto.Util.Tactics.HasBody.
Require Crypto.Util.Tactics.ClearHead.
...
Require coqutil.Tactics.case_match.
Require coqutil.Datatypes.Bool.
Require Stdlib.Numbers.DecimalNat.
...
Require coqutil.Eqb.
Require coqutil.Tactics.ProveInversion.
Require coqutil.InversionRewrite.
...
Require coqutil.Lift1Prop.
Require coqutil.Map.Separation.
...
Require coqutil.Map.SeparationLogic.
...
Require coqutil.Macros.WithBaseName.
Require Rupicola.Lib.Core.
...
Import Crypto.Util.ListUtil.FoldMap.
Import Crypto.Util.ListUtil.Forall.
Import Crypto.Util.ListUtil.Permutation.
Import Crypto.Util.ListUtil.PermutationCompat.
Import Crypto.Util.ListUtil.IndexOf.
Import Crypto.Util.ListUtil.Filter.
Import Crypto.Util.ListUtil.Split.
...
Import Crypto.Util.ZUtil.TruncatingShiftl.
Import Crypto.Util.ZUtil.Land.
...
Import Crypto.Util.ZUtil.Ones.
Import Crypto.Util.ZUtil.LandLorShiftBounds.
Import Crypto.Util.ZUtil.LandLorBounds.
Import Crypto.Util.ZUtil.Tactics.PeelLe.
Import Crypto.Util.Tactics.BreakMatch.
Import Crypto.Util.Tactics.SpecializeBy.
Import Crypto.Util.Tactics.RevertUntil.
Import Crypto.Util.Tactics.HasBody.
Import Crypto.Util.Tactics.Head.
Import Crypto.Util.Tactics.PrintContext.
Import Crypto.Util.Tactics.PrintGoal.
...
Import Crypto.Util.Tactics.ClearHead.
...
Definition get_asm_reg (m : Semantics.reg_state) (reg_available : list REG) : list Z. exact (List.map (Semantics.get_reg m) reg_available). Defined.
Definition R_runtime_input_mem
  ...
  : Prop. exact (exists (stack_placeholder_values : list Z) (output_placeholder_values : list (Z + list Z)),
...
Definition R_runtime_input
           ...
           (m : machine_state)
  : Prop. exact (exists (asm_arguments_out asm_arguments_in : list (Naive.word 64)),
    ...
    /\ R_runtime_input_mem (output_scalars_are_pointers:=output_scalars_are_pointers) frame output_types runtime_inputs stack_size stack_base asm_arguments_out asm_arguments_in runtime_reg m). Defined.
Definition R_runtime_output_mem
           ...
           (asm_arguments_out asm_arguments_in : list (Naive.word 64))
           (m : Semantics.mem_state)
  : Prop. exact (exists (stack_placeholder_values : list Z) (input_placeholder_values : list (Z + list Z)),
    Forall (fun v : Z => (0 <= v < 2 ^ 64)%Z) stack_placeholder_values
    /\ stack_size = List.length stack_placeholder_values
...
Definition R_runtime_output
  ...
  : Prop. exact (exists (asm_arguments_out asm_arguments_in : list (Naive.word 64)),
    ...
    /\ R_runtime_output_mem (output_scalars_are_pointers:=output_scalars_are_pointers) frame runtime_outputs input_types stack_size stack_base asm_arguments_out asm_arguments_in m). Defined.
Definition word_args_to_Z_args
  : list (Naive.word 64 + list (Naive.word 64)) -> list (Z + list Z). exact (List.map (fun v => match v with
...
Definition init_symbolic_state_G (m : machine_state)
  : (symbol -> option Z) * dag -> (symbol -> option Z) * symbolic_state. exact (fun st
